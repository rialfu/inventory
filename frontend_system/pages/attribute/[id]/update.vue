<template>
    <div>
        <h1 class="text-3xl font-bold mb-6">Update Attribute Value</h1>
        <div v-if="notFound">
        ...
        <div class="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md">
            ...
            <form class="">
                <div class="mb-4">
                    ...
                    <input :class="['shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline ', (messageError['name'] !== undefined && messageError['name'] !== ''? 'border-red-500 mb-3':'')]" id="name" type="text" placeholder="Name" @input="changeValue" v-model="nameVal">
...
<script setup>
    ...
    const { id:idParam, name} = route.query
    ...
    async function save() {
        ...
        try {
            if(isNaN(parseInt(idParam))) return;
            const data = await $api('attribute/attribute-value',{
                method:'PUT',
                
                body:{ name:nameVal.value, id: parseInt(idParam) }
            });
            messageError.value = {
                'name':'',
                'message':[]
    ...
    async function fetchData() {
        try {
            ...
            if(data['data'] != undefined && data['data']['attribute_value'] != undefined){
                console.log(data['data']['attribute_value'])
                const index = data['data']['attribute_value'].findIndex((e)=>e['id']== idParam)
                console.log(index)
                if(index == -1){
                    notFound.value = true
                }
