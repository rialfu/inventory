    ...
    <div>
        ...
        <div class="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md">
            ...
            <div class="relative bg-green-600 text-white p-4 rounded-lg shadow-md max-w-sm" v-show="success">
                ...
                <ul class="list-disc list-inside p-0 m-0 text-sm">
                    <li class="mb-1" >Success Update4</li>
                </ul>
            </div>
...
<script setup>
    ...
    const stateParentData = reactive({
        id:null,
        name:null,
    })
    function goBack(){
    ...
    async function save() {
        ...
        try {
            if(true){
                let body = { name: state.form.name,id: state.form.id+""  }
                const data = await $api(`attribute/${stateParentData.id}/attribute-value/`,{
                    method:'PUT',
                    body,
                });
    ...
    async function fetchData() {
        try {
            const res = await $api(`attribute/${parent}/attribute-value/${id}`,{
                method:'GET'
            });
            ...
            let data = res['data']
            if(data['id'] === undefined || data['name'] === undefined || data['attribute_name'] === undefined || data['attribute_name_id'] === undefined){
                return
            }
            stateParentData.id = data['attribute_name_id']
            stateParentData.name =  data['attribute_name']
